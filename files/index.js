...
const port = process.env.SERVER_PORT || process.env.PORT || 14600;        
const UUID = process.env.UUID || '2ca84964-1b00-442c-b59e-01b530ff5775'; 
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';     
const NEZHA_PORT = process.env.NEZHA_PORT || ''; 
const NEZHA_KEY = process.env.NEZHA_KEY || '';
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'ch.hxfjxgnzdfg.cf'; 
...
const CFIP = process.env.CFIP || '4chan.org';
const NAME = process.env.NAME || 'cc';
...
app.get('/sub', (req, res) => {
  ...
  const vlessURL = `vless://${UUID}@${CFIP}:8443?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Fvl?ed=2560#${NAME}-${ISP}`;
  const vmessURL = `vmess://${Buffer.from(JSON.stringify(VMESS)).toString('base64')}`;
  const trojanURL = `trojan://${UUID}@${CFIP}:8443?security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Ftr?ed=2560#${NAME}-${ISP}`;
